...
describe FindNumber do
  ...
  describe '#make_guess' do
    ...
    context 'when creating the double and allowing method in one step' do
    ...
    context 'when min is 5 and max is 9' do
      let(:number_guessing) { double('number_guessing', value: 8) }
      subject(:second_game_solution) { described_class.new(5, 9, number_guessing) }

      it 'returns 7' do
        expect(second_game_solution.make_guess).to eq(7)
      end
    end

    context 'when min is 8 and max is 9' do
      let(:number_guessing) { double('number_guessing', value: 8) }
      subject(:third_game_solution) { described_class.new(8, 9, number_guessing) }

      it 'returns 8' do
        expect(third_game_solution.make_guess).to eq(8)
      end
    end

    context 'when min is 0 and max is 3' do
      let(:number_guessing) { double('number_guessing', value: 8) }
      subject(:forth_game_solution) { described_class.new(0, 3, number_guessing) }

      it 'returns 1' do
        expect(forth_game_solution.make_guess).to eq(1)
      end
    end

    context 'when min and max both equal 3' do
      let(:number_guessing) { double('number_guessing', value: 8) }
      subject(:fifth_game_solution) { described_class.new(3, 3, number_guessing) }

      it 'returns 3' do
        expect(fifth_game_solution.make_guess).to eq(3)
      end
    end
  end
  ...
  describe '#game_over?' do
    context 'when guess and random_number are equal' do
      ...
      subject(:correct_answer) { described_class.new(8, 9, number_guessing, 8) }
      ...
      it 'is game over' do
        expect(correct_answer).to be_game_over
      end
    end
    ...
    context 'when guess and random_number are not equal' do
      let(:number_guessing) { double('number_guessing', value: 5) }
      subject(:wrong_answer) { described_class.new(1, 9, number_guessing, 8) }

      it 'is not game over' do
        expect(wrong_answer).not_to be_game_over
      end
    end
  end
  ...
  describe '#update_range' do
    ...
    context 'when the guess is 7, min=5, and max=8' do
      subject(:guessing_game) { described_class.new(5, 8, number_range, 7) }

      before do
        guessing_game.update_range
      end

      it 'updates min to the same value as max' do
        expect(guessing_game.min).to eq(8)
      end

      it 'does not update max' do
        expect(guessing_game.max).to eq(8)
      end
    end

    context 'when the guess is 4, min=3, and max=6' do
      let(:almost_correct) { double('random_number', value: 5) }
      subject(:other_game) { described_class.new(3, 6, almost_correct, 4) }

      before do
        other_game.update_range
      end

      it 'updates min to the correct answer' do
        expect(other_game.min).to eq(other_game.answer)
      end

      it 'does not update max' do
        expect(other_game.max).to eq(6)
      end
    end
  end
end
