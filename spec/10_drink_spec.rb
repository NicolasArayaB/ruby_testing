...
describe Drink do
  describe '#initialize' do
    context 'when type is specified and ounces is default' do
      ...
      # beverage type.
      subject(:my_pisco) { described_class.new('pisco') }

      it 'is your choice of beverage' do
        expect(my_pisco.type).to eq('pisco')
      end

      it 'has 16 ounces' do
        expect(my_pisco.ounces).to eq(16)
      end
    end
  end
  ...
  describe '#full?' do
    context 'when drink has 16 ounces or more' do
      # Create an explicit subject, using 'described_class' and your choice of
      # beverage type.
      subject(:my_piscola) { described_class.new('piscola') }

      it 'is full' do
        expect(my_piscola).to be_full
      end
    end
    ...
    context 'when drink has less than 16 ounces' do
      ...
      subject(:my_lemon_juice) { described_class.new('lemon juice', 3)}

      it 'is not full' do
        expect(my_lemon_juice).not_to be_full
      end
    end
  end
end
