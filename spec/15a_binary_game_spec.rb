...
describe BinaryGame do
  ...
  describe '#player_input' do
    ...
    context 'when user inputs two incorrect values, then a valid input' do
      before do
        first_invalid_input = '11'
        second_invalid_input = '14'
        valid_input = '3'
        allow(game_input).to receive(:gets).and_return(first_invalid_input, second_invalid_input, valid_input)
      end
  ...
  describe '#verify_input' do
    subject(:verifying_game) { described_class.new(0, 9) }
    ...
    context 'when given a valid input as argument' do
      it 'returns valid input' do
        min = verifying_game.instance_variable_get(:@minimum)
        max = verifying_game.instance_variable_get(:@maximum)
        user_input = 3
        verified_input = verifying_game.verify_input(min, max, user_input)
  ...
  describe '#display_turn_order' do
    ...
    subject(:turn_game) { described_class.new(1, 20) }
    let(:search_display) { instance_double(BinarySearch) }
    ...
    before do
      ...
      allow(turn_game).to receive(:display_guess)
    ...
    it 'increases guess_count by one' do
      expect { turn_game.display_turn_order(search_display) }.to change { turn_game.instance_variable_get(:@guess_count) }.by(1)
    end

    # Method with Outgoing Command -> Test that a message is sent
    it 'sends make_guess' do
      expect(search_display).to receive(:make_guess).once
      turn_game.display_turn_order(search_display)
    end
