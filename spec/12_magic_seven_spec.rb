...
describe MagicSeven do
  ...
  subject(:my_game) { described_class.new(5) }

  describe '#subtract_random_number' do
    context 'when random_number is 5' do
      it 'return 0' do
        random_number = 5
        result = my_game.subtract_random_number(random_number)
  ...
  subject(:your_game) { described_class.new(3) }

  describe '#play' do
    context 'when the random number is 5' do
      it 'will return 7' do
        result = your_game.play
        expect(result).to eq(7)
      end
    end

  subject(:our_game) { described_class.new(8) }
    context 'when the random number is 8' do
      it 'will return 7' do
        result = our_game.play
        expect(result).to eq(7)
      end
    end

  subject(:other_game) { described_class.new(6) }
    context 'when the random number is 6' do
      it 'will return 7' do
        result = other_game.play
        expect(result).to eq(7)
      end
    end
  end
